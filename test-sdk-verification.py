...
══════════════════════════════════════════════════════════════
 ...
 This script uses the actual citizenofthecloud Python SDK
...
def main():
    ...
    # 7a: Wrong key — Agent A's ID signed with Agent B's key
    info("Test 7a: Wrong private key (impersonation attempt)")
    wrong_identity = CloudIdentity(
        cloud_id=agent_a["cloud_id"],
        private_key=keys_b["private_key"],  # Wrong key!
        registry_url=REGISTRY_URL,
    )
    ...
    # 7e: Partial headers (only Cloud ID, no signature)
    ...
    # Use the headers from step 5 (now 2+ seconds old)
    clear_cache()
